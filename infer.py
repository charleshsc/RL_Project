...
if __name__ == '__main__':
    ...
    checkpoint = os.path.join('./run', algorithm + '_' + env_name[:-3], 'best_model.pth')

    if not os.path.isfile(checkpoint):
        raise ValueError("Please follow the instructions to infer the model")

    if args.algorithm == 'A3C':
        net = ActorCritic(state_dim, action_dim)
        checkpoint = torch.load(checkpoint)
        net.load_state_dict(checkpoint)
        eval_policy(net, env_name, seed, eval_episodes=1, is_render=True)
